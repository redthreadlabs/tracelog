...
// Write a client-originated transaction record to the JSONL output.
// Validates and sanitizes the input to conform to the transaction schema.
// Designed for server endpoints that receive span-shaped perf data from clients.
Agent.prototype.writeClientTransaction = function (input) {
  ...
  if (!id || !trace_id) return; // Required fields missing or malformed

  const transaction = {
    id,
    trace_id,
    name: typeof input.name === 'string' ? input.name.slice(0, 1024) : 'unknown',
    type: typeof input.type === 'string' ? input.type.slice(0, 1024) : 'client-perf',
    ...
    result: typeof input.result === 'string' ? input.result.slice(0, 1024) : 'unknown',
  ...
  if (typeof input.parent_id === 'string' && HEX_16.test(input.parent_id)) {
    transaction.parent_id = input.parent_id;
  }

  if (input.context && typeof input.context === 'object') {
    transaction.context = _sanitizeContext(input.context);
  }

  this._apmClient.sendTransaction(transaction);
};

// Write a client-originated span record to the JSONL output.
// Validates and sanitizes the input to conform to the span schema.
Agent.prototype.writeClientSpan = function (input) {
  if (!this._apmClient || !input) return;
  ...
  if (!id || !trace_id || !transaction_id || !parent_id) return; // Required fields missing or malformed
  ...
  if (input.context && typeof input.context === 'object') {
    span.context = _sanitizeContext(input.context);
  }
