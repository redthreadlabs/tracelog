...
Instrumentation.prototype._encodeAndSendSpan = function (span) {
  ...
  // Spans follow their transaction's channel, but only when the transaction
  // has a *resolved* name (custom or framework-set) at the time the span is
  // sent. Resolve this now, synchronously: the encode callback below may run
  // after the transaction is named, which would make routing dependent on
  // encode timing. The `name` getter must not be used here: until the
  // transaction ends it falls back to '<METHOD> unknown route (unnamed)',
  // which would spuriously match rules aimed at unmatched-route traffic and
  // divert every in-flight request's spans. Spans of transactions that are
  // only named at end (e.g. Express routes) stay on the default channel.
  const spanTrans = span.transaction;
  const spanChannel = agent._channelForTransactionName(
    spanTrans && (spanTrans._customName || spanTrans._defaultName),
  );
      ...
      if (!payload) {
        ...
        if (agent._apmClient) {
          if (
            ...
            agent._apmClient.sendToChannel(spanChannel, 'span', payload);
          } else {
